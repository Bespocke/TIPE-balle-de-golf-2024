...
class Fluid:
    def __init__(self, shape, *quantities, pressure_order=1, advect_order=3):
        ...
        # Prototyping is simplified by dynamically creating advected quantities as needed.
        ...
        self.ball_rotation_speed = 0.5  # Vitesse angulaire de rotation de la balle

        # Alvéoles
        self.alveoles = np.zeros(shape, dtype=bool)
        # Définir les positions des alvéoles (exemple ici, au centre)
        center = np.floor_divide(shape, 2)
        alveole_radius = 8
        self.alveoles[center[0] - alveole_radius:center[0] + alveole_radius,
                      center[1] - alveole_radius:center[1] + alveole_radius] = True
    ...
    def step(self, dt=1):
        ...
        # Apply advection to each axis of the velocity field and each user-defined quantity.
        ...
        # Compute the jacobian at each point in the velocity field to extract curl and divergence.
        ...
        # Mettre à jour le masque de la balle avec la nouvelle position et les alvéoles
        self.ball_mask = np.linalg.norm(self.indices - ball_center[:, None, None], axis=0) <= self.ball_radius
        self.ball_mask |= self.alveoles  # Combinaison avec le masque des alvéoles

        # Interagir avec le fluide en définissant la vitesse autour de la balle et des alvéoles
