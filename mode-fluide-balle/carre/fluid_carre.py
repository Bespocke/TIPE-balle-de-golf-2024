...
class Fluid:
 ...
 # Créer un carré au lieu d'une balle
        self.square_size = 80
        square_center = np.floor_divide(shape, 2)
        self.ball_mask = (
            (abs(self.indices[0] - square_center[0]) <= self.square_size // 2) &
            (abs(self.indices[1] - square_center[1]) <= self.square_size // 2))
    ...
    def step(self):
        ...
        solid_velocity[:, self.ball_mask] = 0  # La balle est immobile, donc sa vitesse est nulle
