...
class Masse:
    def __init__(self, masse, rayon, pos, vit, acc, ang, couleur):
        self.x, self.y = pos
        vx, vy = vit
        self.vit_x, self.vit_y = vx * np.cos(ang), vy * np.sin(ang)
#        self.vit_x, self.vit_y = vx, vy
...
class main:
    ...
    def update(self, masse):
        global premier_contact
        global carry
        temps_actuel = pygame.time.get_ticks()  # Obtenir le temps actuel
        temps_ecoule_secondes = temps_actuel / 1000.0  # Temps écoulé depuis le début de la simulation en secondes

        masse.rota()

        if masse.y == self.Sol.altitude - self.Sol.hauteur//2 - masse.rayon + 1 and masse.x >= 300:
            ...
            masse.x += masse.vit_x
            if masse.spin:
                masse.acc_y = -self.frot * masse.vit_y / masse.masse + self.grav - 1.1 ** temps_ecoule_secondes
            else: 
                masse.acc_y = -self.frot * masse.vit_y / masse.masse + self.grav - 0.6
            masse.vit_y += masse.acc_y
            masse.y += masse.vit_y 

        if (masse.y + masse.rayon) >= self.Sol.altitude - self.Sol.hauteur//2 and not masse.x >= 300:
            masse.vit_y *= -1
            masse.y = self.Sol.altitude - self.Sol.hauteur//2 - masse.rayon + 1
        
        if (masse.y + masse.rayon) >= self.Sol.altitude - self.Sol.hauteur//2 and masse.x >= 300:
...
#                pygame.draw.line(self.screen, (124, 252, 0), (self.width // 2 - self.Sol.largeur // 2, self.Sol.altitude), (self.width // 2 + self.Sol.largeur // 2, self.Sol.altitude), self.Sol.hauteur)
                pygame.draw.rect(self.screen, (124, 252, 0), (0, self.Sol.altitude, self.width, self.height - self.Sol.altitude))
                ...
                if len(termine) == len(self.Masse) and bool == True:
                    ...
                    print_blue("carry :" + str(carry))
                    print("\n")
                    print_green("altitude maximale :"+ str(max(D_y) - min(D_y)))
                    print("\n")
                    bool = False 
